...
\begin{lemma}\label{8.11}
    Let $G=(V,E)$ be a graph and let $D=(S,T)$ be a nice treedecomposition of $tw(\color{red}D\color{black})=k$. An independent set in $G$ of size $\alpha(G)$ can be computed in $O(k2^{k+1}n)$, where $n=|V(G)|$.
\end{lemma}
\begin{proof}
    ...
    Note $G_s=G$. This can be computed in $O(2^{k+1})$.\\
    ...
    \begin{enumerate}
        ...
        If we know $C_j$, we can compute $C_i$ in $O(k2^{k+1})$.
    \end{enumerate}
\end{proof}