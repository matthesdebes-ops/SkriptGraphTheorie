...
\begin{lemma}\label{8.11}
    Let $G=(V,E)$ be a graph and let $D=(S,T)$ be a nice treedecomposition of $tw(\color{red}D\color{black})=k$. An independent set in $G$ of size $\alpha(G)$ can be computed in $\mathcal{O}(k2^{k+1}n)$, where $n=|V(G)|$.
\end{lemma}
\begin{proof}
    For $i\in V(T)$, let $V_i=X_i\cup\bigcup_{j \text{ descendent of } i \text{ in } T}X_j$. Set $G_i=G[V_i]$.
     \begin{center}
            \begin{tikzpicture}[
            vertex/.style={circle, draw, fill=gray!20, minimum size=5mm, inner sep=0pt}
            ]
            ...
            \node[vertex] (d) at (-1,6) {};
            \node[vertex] (e) at (0,6) {};
            \node[vertex] (f) at (2,6) {$i$};
            \node[vertex] (g) at (0,5) {};
            \node[vertex] (h) at (2,5) {};
            \node[vertex] (i) at (2,4) {$l$};
