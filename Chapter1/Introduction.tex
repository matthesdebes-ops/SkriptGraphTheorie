...
\begin{example}\label{1.3}
    ...
    \begin{itemize}
        ...
        \item The \emph{complete graph}\index{complete graph} $K_n$ has $n$ vertices and each pair of vertices is connected.
        ...
        \item The \emph{cycle}\index{cycle} $C_n$ has $n$ vertices, which are cyclically connected.
...
\begin{definition}\label{1.7}
    Let $G=(V,E)$ be a graph. A \textbf{subgraph}\index{subgraph} of $G$ is a graph $H=(V',E')$ with $V'\subseteq V$ and $E'\subseteq E\cap \begin{pmatrix} V' \\ 2 \end{pmatrix}$ where $\begin{pmatrix}
        S \\ K
...
Let $v_i$ be vertices of a graph $G$
    \begin{itemize}
        \item A \textbf{walk}\index{walk} is a sequence of vertices $v_i$ s.t. $\{v_i,v_{i+1}\}\in E(G) \forall i\in \{1,...,k-1\}$
        \item The \textbf{length}\index{length} of a walk is $k$ (number of vertices in the walk)
        ...
        \item A \textbf{path}\index{path} is a walk for which $v_1,...,v_k$ are pairwise distinct
    \end{itemize}
    \begin{center}
        \begin{tikzpicture}[
            vertex/.style={circle, draw, fill=gray!20, minimum size=2mm, inner sep=0pt}
...
Let $G$ be a graph
    \begin{itemize}
        \item A graph $G$ is \textbf{connected}\index{connected graph} if there is a walk between any pair of its vertices
        \item A \textbf{(connected) component}\index{component} of $G$ is an inclusion wise maximal connected subgraph
    \end{itemize}
\end{definition}
