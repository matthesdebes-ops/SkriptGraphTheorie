...
\begin{definition}
    ...
    \end{itemize}
    The \textbf{value} of a flow $f$ is $val(f)=\sum\limits_{e\in\delta^-(v)} f(e)=\sum\limits_{e\in\delta^+(v)}f(e)$
\end{definition}
\begin{remark}
...
\begin{theorem}[max flow-min cut theorem]
    Let $(G,s,t,c)$ be a flow network. Then $\max\{val(f):f\text{ flow}\}=\min\{c(S,T):(S,T) \text{ is a } (s,t)\text{-cut}\}$.
\end{theorem}
\vspace{0.5cm}
\begin{lemma}
    Let $f$ be a flow, $(S,T)$ be a $(s,t)$-cut. Then $val(f)=f(\delta^+(S))-f(\delta^-(S))$.
\end{lemma}