...
\begin{definition}\label{2.1}
    A \textbf{tree}\index{tree} is a graph that is connected and does not contain a cycle (a graph "contains a cycle" if it has a subgraph isomorphic to $C_n$ for some $n\geq3$).\\
    A \textbf{forest}\index{forest} is a graph that does not contain a cycle.\\
    A \textbf{leaf}\index{leaf} is a vertex of a tree with degree $1$.
\end{definition}
...
\begin{lemma}\label{2.3}
    Every tree $T$ with $n\geq 2$ vertices has at least one leaf.
\end{lemma}
\begin{proof}
...
\begin{proof}
    ...
    Induction on $n$:\\
    $n=1$ is trivial\\
    Let $n\geq2$. By Lemma \ref{2.3}, $G$ has a leaf $v$. Removing $v$ and the unique edge incident to $v$ yields a tree $G'$. By induction $G'$ has $n-2$ edges so $G$ has $n-1$ edges.\par
    \vspace{0.3cm}
    "$2.\implies3.$"\\
    ...
    Adding an edge creates a cycle: Let $u,v\in V(G)$ non adjacent. There is one path between the two. Now adding an edge between $u,v$ creates a second path $\lightning$\par
    \vspace{0.3cm}
    "$5.\implies 1.$\\
...
\end{theorem}
\vspace{0.5cm}
\begin{corollary}[Cayley's formula]\label{2.7}
    The number of labelled spanning trees of $K_n$ is $n^{n-2}$ if $n\geq 2$.
\end{corollary}
\begin{proof}
    Define $Q$ as in Theorem \ref{2.6}. \\
    $Q_{11}=\begin{pmatrix}
        n-1 & 1 & \cdots & 1\\
        1 & \ddots & & \vdots\\
        \vdots & & \ddots & 1\\
